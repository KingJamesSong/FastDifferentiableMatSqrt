...
a = th.from_numpy(np.array(a).astype(float))
pade_p = th.from_numpy(np.array(pade_p).astype(float)).half()
pade_q = th.from_numpy(np.array(pade_q).astype(float)).half()

def pade_approximant_inverse(p,I):
    p_sqrt = pade_p[0]*I
    q_sqrt = pade_q[0]*I
    p_app = I - p
    p_hat = p_app
    ...
    #return th.linalg.inv(q_sqrt).bmm(p_sqrt)
    return th.linalg.solve(p_sqrt,q_sqrt)
    #return th.linalg.inv(p_sqrt).mm(q_sqrt)

class TaylorSquareRootInverse(th.autograd.Function):
    @staticmethod
    def forward(ctx, M):
        normM = th.norm(M,dim=[1,2]).reshape(M.size(0),1,1)
        I = th.eye(M.size(1), requires_grad=False, device=M.device).reshape(1,M.size(1),M.size(1)).repeat(M.size(0),1,1).type(M.dtype)
        #M_sqrt = taylor_expansion(M/normM,I)
        M_sqrt_inv = pade_approximant_inverse(M / normM, I)
        M_sqrt_inv = M_sqrt_inv / th.sqrt(normM)
    ...
    def backward(ctx, grad_output):
        ...
        norm_sqrt_inv = th.norm(M_sqrt_inv)
        a = M_sqrt_inv / norm_sqrt_inv
        q = grad_lya / norm_sqrt_inv
        for i in range(8):
            a_2 = a.bmm(a)
            q = 0.5 * (q.bmm(3.0*I-a_2)-a_2.bmm(q)+a.bmm(q).bmm(a))
            a = 0.5 * a.bmm(3.0 * I - a_2)
        grad_input = 0.5 * q
        return grad_input

inverse_square_root=TaylorSquareRootInverse.apply
