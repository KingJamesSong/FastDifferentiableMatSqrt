...
def get_loader(image_dir, crop_size=216, resize=216, 
    ...
    #else:
    #    transform.append(T.TenCrop(crop_size))
    #    transform.append(T.Lambda(lambda crops: torch.stack([T.ToTensor()(crop) for crop in crops])))
    #    transform.append(T.Lambda(lambda crops: torch.stack([T.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))(crop) for crop in crops])))
